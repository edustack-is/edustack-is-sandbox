...
function getSubstitutionBadge(type: SubstitutionData['type']) {
    switch (type) {
        case 'CANCELLED': return <Badge variant="destructive" className="text-[10px] px-1 py-0">Odpadá</Badge>;
        case 'SUBSTITUTION': return <Badge className="text-[10px] px-1 py-0 bg-amber-500">Suplování</Badge>;
        case 'MERGED': return <Badge className="text-[10px] px-1 py-0 bg-violet-500">Spojeno</Badge>;
        case 'ROOM_CHANGE': return <Badge className="text-[10px] px-1 py-0 bg-sky-500">Změna učebny</Badge>;
        case 'SUBJECT_CHANGE': return <Badge className="text-[10px] px-1 py-0 bg-orange-500">Změna předmětu</Badge>;
        default: return null;
    }
}
...
export const TimetableGrid: React.FC<TimetableGridProps> = ({
    ...
    const slots = timeSlots && timeSlots.length > 0
        ? timeSlots.sort((a, b) => a.lessonNumber - b.lessonNumber)
        : DEFAULT_SLOTS;

    // Determine how many lessons to show
    const maxUsed = events.length > 0
        ? Math.max(...events.map(e => e.lessonNumber))
        : 0;
        ...
        <TooltipProvider delayDuration={200}>
            <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                    ...
                    <tbody>
                        {slots.filter(s => s.lessonNumber <= lessonCount).map(slot => (
                            <tr key={slot.lessonNumber}>
                                ...
                                {DAYS.map(day => {
                                    ...
                                    return (
                                        <td
                                            key={day.num}
                                            className={`border-b p-0.5 align-top transition-colors ${editable && !event ? 'cursor-pointer hover:bg-muted/20' : ''
                                                }`}
                                            onClick={() => {
                                            ...
                                            {event ? (
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <div
                                                            className={`rounded-md border p-1.5 min-h-[52px] transition-all ${todaySub
                                                                ? 'border-dashed opacity-80'
                                                                : ''
                                                                } ${getSubjectColor(event.subject.template.code)} ${editable ? 'cursor-pointer hover:shadow-md' : ''
                                                                }`}
                                                        >
                                                    ...
                                                    <TooltipContent side="right" className="max-w-xs">
                                                        <div className="space-y-1">
                                                            <div className="font-semibold">{event.subject.template.name}</div>
                                                            <div className="text-xs">
                                                                Učitel: {event.teacherProfile.user.firstName} {event.teacherProfile.user.lastName}
                                                            </div>
                                                            <div className="text-xs">
                                                                Třída: {event.classroom.name}
                                                            </div>
                                                            {event.room && (
                                                                <div className="text-xs">Učebna: {event.room.name}</div>
                                                            )}
                                                            ...
                                                            {todaySub && (
                                                                <div className="mt-1 pt-1 border-t">
                                                                    ...
                                                                    {todaySub.substituteTeacher && (
                                                                        <div className="text-xs">
                                                                            → {todaySub.substituteTeacher.user.firstName} {todaySub.substituteTeacher.user.lastName}
                                                                        </div>
                                                                    )}
                                                                    {todaySub.note && (
                                                                        <div className="text-xs italic">{todaySub.note}</div>
                                                                    )}
                                                                </div>
                                                            )}
                                                        </div>
                                                ...
                                                <div className={`min-h-[52px] rounded-md ${editable ? 'border border-dashed border-transparent hover:border-muted-foreground/20' : ''
                                                    }`} />
                                            )}
                                        </td>
                                    );
