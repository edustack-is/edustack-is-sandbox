...
export class ClassBookService {
    ...
    async upsertEntry(userId: string, schoolId: string, data: {
    ...
    async signEntry(userId: string, entryId: string, ipAddress?: string) {
        const entry = await this.prisma.classBookEntry.findUnique({ where: { id: entryId } });
        if (!entry) throw new NotFoundException('Záznam nenalezen');
        if (entry.teacherId !== userId) throw new ForbiddenException('Můžete podepsat pouze své záznamy');
    ...
    async getEntriesForRange(schoolId: string, classroomId: string, dateFrom: string, dateTo: string) {
    ...
    async generatePrintHtml(schoolId: string, classroomId: string, dateFrom: string, dateTo: string) {
        const entries = await this.getEntriesForRange(schoolId, classroomId, dateFrom, dateTo);
...
<p>Období: ${dateFrom} – ${dateTo}</p>`;
        ...
        for (const [dateKey, dayEntries] of byDate) {
            ...
            const dayLabel = d.toLocaleDateString('cs-CZ', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
...
<tr><th>Hod.</th><th>Předmět</th><th>Učitel</th><th>Probírané učivo</th><th>Poznámky</th><th>Nepřítomnos</th><th>Podpis</th></tr>`;
            for (const e of dayEntries) {
                const teacherName = e.teacher ? `${e.teacher.lastName} ${e.teacher.firstName}` : '-';
                const sig = e.signature ? `<span class="sig">✓ ${e.signature.signedAt.toISOString().slice(0, 16)}</span>` : '-';
                html += `<tr>
...
<td>${sig}</td>
    ...
    async getAttendanceForLesson(schoolId: string, classroomId: string, date: string, lessonNumber: number) {
        const d = new Date(date);
