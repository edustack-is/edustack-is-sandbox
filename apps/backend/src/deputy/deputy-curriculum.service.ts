...
export class DeputyCurriculumService {
    ...
    async duplicateCurriculumVersion(
        ...
        await this.audit(actorId, 'DUPLICATE_CURRICULUM_VERSION', 'CurriculumVersion', newVersion.id, {
            sourceVersionId,
    ...
    async compareCurriculumVersions(schoolId: string, versionAId: string, versionBId: string) {
        ...
        type EntryMap = Map<string, Map<string, { hoursPerWeek: number; rvpDescription: string | null; svpApproach: string | null; needsComputerLab: boolean }>>;

        const buildMap = (entries: any[]): EntryMap => {
            const map: EntryMap = new Map();
            for (const e of entries) {
                if (!map.has(e.subjectTemplateId)) map.set(e.subjectTemplateId, new Map());
        ...
        for (const e of [...versionA.entries, ...versionB.entries]) {
            subjectInfo[e.subjectTemplateId] = { name: e.subjectTemplate.name, code: e.subjectTemplate.code };
            gradeInfo[e.gradeLevelId] = { name: e.gradeLevel.name, levelNumber: e.gradeLevel.levelNumber };
        }
        ...
        for (const subjectId of allSubjectIds) {
            ...
            if (!inA && inB) {
                // Added in B
                const grades = Array.from(mapB.get(subjectId)!.entries()).map(([glId, data]) => ({
                    gradeLevelId: glId,
                    ...gradeInfo[glId],
                    hoursPerWeek: data.hoursPerWeek,
                }));
                added.push({ subjectId, ...info, grades, totalHours: grades.reduce((s, g) => s + g.hoursPerWeek, 0) });
            } else if (inA && !inB) {
                // Removed from B
                const grades = Array.from(mapA.get(subjectId)!.entries()).map(([glId, data]) => ({
                    gradeLevelId: glId,
                    ...gradeInfo[glId],
                    hoursPerWeek: data.hoursPerWeek,
                }));
                removed.push({ subjectId, ...info, grades, totalHours: grades.reduce((s, g) => s + g.hoursPerWeek, 0) });
            } else if (inA && inB) {
                ...
                for (const glId of allGrades) {
                    ...
                    if (!a && b) {
                        hasChanges = true;
                        gradeDiffs.push({ gradeLevelId: glId, ...gInfo, hoursA: 0, hoursB: b.hoursPerWeek, diff: b.hoursPerWeek, status: 'added' });
                    } else if (a && !b) {
                        hasChanges = true;
                        gradeDiffs.push({ gradeLevelId: glId, ...gInfo, hoursA: a.hoursPerWeek, hoursB: 0, diff: -a.hoursPerWeek, status: 'removed' });
                    } else if (a && b) {
                        const diff = b.hoursPerWeek - a.hoursPerWeek;
                        if (diff !== 0) hasChanges = true;
                        gradeDiffs.push({
                            gradeLevelId: glId, ...gInfo,
                            hoursA: a.hoursPerWeek, hoursB: b.hoursPerWeek, diff,
                ...
                if (hasChanges) {
                    changed.push({ subjectId, ...info, grades: gradeDiffs, totalHoursA: totalA, totalHoursB: totalB, totalDiff: totalB - totalA });
                } else {
                    unchanged.push({ subjectId, ...info, totalHours: totalA, gradeCount: gradeDiffs.length });
                }
            }
        }
        ...
        return {
            versionA: { id: versionA.id, name: versionA.name, validFrom: versionA.validFrom, entryCount: versionA.entries.length },
            versionB: { id: versionB.id, name: versionB.name, validFrom: versionB.validFrom, entryCount: versionB.entries.length },
            ...
            summary: {
                ...
                totalHoursA: [...mapA.values()].reduce((s, m) => s + [...m.values()].reduce((s2, e) => s2 + e.hoursPerWeek, 0), 0),
                totalHoursB: [...mapB.values()].reduce((s, m) => s + [...m.values()].reduce((s2, e) => s2 + e.hoursPerWeek, 0), 0),
            },
            added,
            removed,
            changed,
    ...
    async createStaffWorkload(
        ...
        const membership = await this.prisma.schoolMembership.findFirst({
            where: { userId: data.userId, schoolId, status: 'ACTIVE', role: { notIn: ['STUDENT', 'PARENT'] } },
        });
        if (!membership) throw new NotFoundException('User is not an active staff member of this school.');
        ...
        await this.audit(actorId, 'CREATE_STAFF_WORKLOAD', 'StaffWorkload', workload.id, data);
        return workload;
    }
    ...
    async updateStaffWorkload(
        ...
        if (data.validFrom !== undefined) updateData.validFrom = new Date(data.validFrom);
        if (data.teachingLoad !== undefined) updateData.teachingLoad = data.teachingLoad;
        ...
        await this.audit(actorId, 'UPDATE_STAFF_WORKLOAD', 'StaffWorkload', workloadId, data, workload);
        return updated;
    }

    async deleteStaffWorkload(actorId: string, schoolId: string, workloadId: string) {
        const workload = await this.prisma.staffWorkload.findFirst({
            where: { id: workloadId, academicYear: { schoolId } },
        });
        if (!workload) throw new NotFoundException('Staff workload not found.');
        ...
        await this.audit(actorId, 'DELETE_STAFF_WORKLOAD', 'StaffWorkload', workloadId, {}, workload);
        return { deleted: true };
    }
    ...
    async saveStaffSubjectAssignments(
        ...
        await this.audit(actorId, 'SAVE_STAFF_SUBJECT_ASSIGNMENTS', 'StaffWorkload', workloadId, { assignments });
        return result;
    }
