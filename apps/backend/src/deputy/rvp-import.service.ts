...
const RvpSubjectSchema = z.object({
    name: z.string().describe('Full Czech subject name, e.g. "Český jazyk a literatura"'),
    code: z.string().describe('Short abbreviation, e.g. "ČJ", "M", "AJ", "PRV"'),
    educationalArea: z.string().optional().describe('Educational area (vzdělávací oblast), e.g. "Jazyk a jazyková komunikace"'),
});

const RvpAllocationSchema = z.object({
    subjectName: z.string().describe('Subject name – must exactly match one from the subjects list'),
    gradeLevel: z.number().int().min(1).max(9).describe('Grade level (ročník) 1–9'),
    hoursPerWeek: z.number().min(0).describe('Weekly hours allocation (hodinová dotace za týden)'),
    rvpDescription: z.string().optional().describe('Expected outcomes / key competencies from RVP for this subject × grade'),
});
...
const RvpExtractionResultSchema = z.object({
    ...
    subjects: z.array(RvpSubjectSchema).describe('All subjects found in the document'),
    allocations: z.array(RvpAllocationSchema).describe('Hourly allocations per subject × grade'),
    totalGrades: z.number().int().describe('Number of grade levels (e.g. 9 for ZŠ, 4/6/8 for gymnázium)'),
    notes: z.string().optional().describe('Any important notes about the curriculum structure'),
});
...
export class RvpImportService {
    ...
    async extractTextFromUrl(url: string): Promise<string> {
        try {
            ...
            if (!response.ok) {
                throw new BadRequestException(`Nepodařilo se stáhnout dokument (HTTP ${response.status}).`);
            }
...
${truncated}`,
    ...
    async buildPreview(
        ...
        // Match grade levels
        const allGrades = [...new Set(extraction.allocations.map((a) => a.gradeLevel))].sort(
            (a, b) => a - b,
        );
    ...
    async confirmImport(
        ...
        for (const alloc of data.allocations) {
            ...
            try {
                ...
                this.logger.warn(`Skipped duplicate entry: ${alloc.subjectName} × grade ${alloc.gradeLevel}`);
            }
        }
        ...
        await this.prisma.auditLog.create({
            data: {
                ...
                newValues: {
                    ...
                    subjectsMapped: data.subjectMappings.filter((s) => s.existingId).length,
                    gradeLevelsCreated: data.gradeMappings.filter((g) => !g.existingId).length,
                    entriesCreated,
                },
            },
        });
    ...
    private async trackUsage(userId: string, schoolId: string | null, usage: any) {
        if (!usage) return;
        try {
