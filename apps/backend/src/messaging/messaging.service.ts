...
export class MessagingService {
    ...
    async getConversations(userId: string, schoolId: string) {
        ...
        return participations.map(p => {
            ...
            const unreadCount = lastMessage && (!p.lastReadAt || new Date(lastMessage.createdAt) > new Date(p.lastReadAt))
    ...
    async getMessages(conversationId: string, userId: string, limit = 50, offset = 0) {
        ...
        if (!participant) throw new ForbiddenException('Nemáte přístup k této konverzaci.');
    ...
    async sendMessage(conversationId: string, senderId: string, content: string) {
        if (!content.trim()) throw new BadRequestException('Zpráva nemůže být prázdná.');
    ...
    async createConversation(
        ...
        if (recipientIds.length === 0) throw new BadRequestException('Vyberte alespoň jednoho příjemce.');

        // For DIRECT conversations, check if one already exists between these two users
        if (type === 'DIRECT' && recipientIds.length === 1) {
            const existing = await this.findExistingDirectConversation(creatorId, recipientIds[0], schoolId);
            if (existing) {
        ...
        const allParticipantIds = [creatorId, ...recipientIds.filter(id => id !== creatorId)];
    ...
    private async findExistingDirectConversation(user1: string, user2: string, schoolId: string) {
    ...
    private async getTeacherRecipients(userId: string, schoolId: string) {
        ...
        const parentRelations = await this.prisma.parentStudent.findMany({
            ...
            include: {
                parent: { select: { id: true, firstName: true, lastName: true, email: true, avatarUrl: true } },
            },
        });
    ...
    async getAvailableClassrooms(userId: string, schoolId: string) {
        ...
        if (!membership || !['PRINCIPAL', 'DEPUTY', 'ADMIN', 'DIRECTOR', 'TEACHER'].includes(membership.role)) {
            return [];
        }

        if (['PRINCIPAL', 'DEPUTY', 'ADMIN', 'DIRECTOR'].includes(membership.role)) {
     ...
     */
    async createClassBroadcast(senderId: string, schoolId: string, classroomId: string, subject: string, message: string) {
        ...
        return this.createConversation(
            senderId, schoolId, uniqueRecipients, subject, 'CLASS_BROADCAST', classroomId, message,
        );
    }
     ...
     */
    async createSchoolBroadcast(senderId: string, schoolId: string, subject: string, message: string) {
        const memberships = await this.prisma.schoolMembership.findMany({
