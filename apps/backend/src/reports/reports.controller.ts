...
export class ReportsController {
    ...
    @Get('grades/school')
    @ApiOperation({ summary: 'Statistiky prospěchu školy', description: 'Přehled průměrů a počtu známek per třída.' })
    @ApiQuery({ name: 'semesterId', required: false, description: 'ID semestru (volitelné)' })
    ...
    @Get('csi')
    @ApiOperation({ summary: 'Výkaz pro ČŠI (JSON)', description: 'Data pro Českou školní inspekci: personální zajištění, třídy, klasifikace, docházka.' })
    @ApiQuery({ name: 'academicYearId', required: false, description: 'ID školního roku (volitelné)' })
    async csiReport(
        @Req() req: any,
    ...
    @Get('csi/print')
    @ApiOperation({ summary: 'Výkaz pro ČŠI (HTML)', description: 'Tisknutelná HTML verze výkazu pro ČŠI.' })
    @ApiQuery({ name: 'academicYearId', required: false, description: 'ID školního roku (volitelné)' })
    ...
    @Get('msmt')
    @ApiOperation({ summary: 'Výkaz pro MŠMT (JSON)', description: 'Data pro Ministerstvo školství: žáci, učitelé, ročníky, předměty, zázemí.' })
    ...
    @Get('msmt/print')
    @ApiOperation({ summary: 'Výkaz pro MŠMT (HTML)', description: 'Tisknutelná HTML verze výkazu pro MŠMT.' })
    @ApiProduces('text/html')
