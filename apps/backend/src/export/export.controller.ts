...
export class ExportController {
  ...
  @Get('students')
  @ApiOperation({
    summary: 'Export studentů',
    description: 'Stáhne seznam studentů ve zvoleném formátu.',
  })
  @ApiResponse({
    status: 200,
    description: 'Export studentů ve zvoleném formátu.',
  })
  @ApiQuery({
    name: 'format',
    enum: ['csv', 'xml', 'json'],
    required: false,
    description: 'Výstupní formát (výchozí: csv)',
  })
  ...
  @Get('grades')
  @ApiOperation({
    summary: 'Export známek',
    description: 'Stáhne známky, volitelně filtrované dle třídy.',
  })
  ...
  async exportGrades(
    ...
    this.ensureTenant(req);
    const fmt = this.validateFormat(format);
    const data = await this.exportService.getGradesData(
      req.user.schoolId,
      classroomId,
    );
  ...
  @Get('attendance')
  @ApiOperation({
    summary: 'Export docházky',
    description: 'Stáhne záznamy docházky s filtry.',
  })
  ...
  async exportAttendance(
    ...
    this.ensureTenant(req);
    const fmt = this.validateFormat(format);
    const data = await this.exportService.getAttendanceData(
      req.user.schoolId,
      classroomId,
      dateFrom,
      dateTo,
  ...
  @Get('schedule')
  @ApiOperation({
    summary: 'Export rozvrhu',
    description: 'Stáhne rozvrhové události.',
  })
  @ApiQuery({
    name: 'format',
    enum: ['csv', 'xml', 'json'],
  ...
  async exportSchedule(
    ...
    this.ensureTenant(req);
    const fmt = this.validateFormat(format);
    const data = await this.exportService.getScheduleData(
      req.user.schoolId,
      classroomId,
    );
  ...
  @Get('classbook')
  @ApiOperation({
    summary: 'Export třídní knihy',
    description: 'Stáhne záznamy třídní knihy.',
  })
  @ApiQuery({
    name: 'format',
    enum: ['csv', 'xml', 'json'],
  ...
  async exportClassbook(
    ...
    const fmt = this.validateFormat(format);
    const data = await this.exportService.getClassbookData(
      req.user.schoolId,
      classroomId,
      dateFrom,
      dateTo,
  ...
  private sendExport(
    ...
    switch (format) {
      case 'csv':
        content = this.exportService.toCsv(data);
        break;
      case 'xml':
        content = this.exportService.toXml(data, entity, itemName);
        break;
      case 'json':
        content = this.exportService.toJson(data);
        break;
    }
