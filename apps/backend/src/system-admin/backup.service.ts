...
export class BackupService {
    ...
    async listBackups(): Promise<Array<{ filename: string; size: number; createdAt: string; storage: 'R2' | 'LOCAL' }>> {
        const backups: any[] = [];
    ...
    async uploadBackup(file: Express.Multer.File): Promise<{ filename: string; storage: 'R2' | 'LOCAL' }> {
