...
const MALE_FIRST = [
    ...
    'Roman', 'Milan', 'Vladimír', 'Aleš', 'Richard', 'Patrik', 'Stanislav', 'Dominik', 'Štěpán', 'Václav',
];
const FEMALE_FIRST = [
    ...
    'Michaela', 'Simona', 'Andrea', 'Kristýna', 'Denisa', 'Nikola', 'Adéla', 'Karolína', 'Gabriela', 'Natálie',
];
const LAST_NAMES = [
    ...
    'Urban', 'Vlček', 'Hrubý', 'Němec', 'Pokorný', 'Beneš', 'Mach', 'Holub', 'Kadlec', 'Strnad',
];

const TEACHER_DEGREES = ['Mgr.', 'Mgr.', 'Mgr.', 'RNDr.', 'PhDr.', 'Ing.', 'PaedDr.', 'MgA.'];
const TEACHER_APPROBATIONS = [
    'Český jazyk a literatura', 'Matematika', 'Anglický jazyk', 'Fyzika',
    'Chemie', 'Přírodopis', 'Dějepis', 'Zeměpis', 'Tělesná výchova',
    'Informatika', 'Hudební výchova', 'Výtvarná výchova', 'Občanská výchova',
    'Německý jazyk', '1. stupeň ZŠ', 'Speciální pedagogika',
];
const GRADE_DESCRIPTIONS = [
    'Písemná práce', 'Ústní zkoušení', 'Domácí úkol', 'Projekt', 'Test',
    'Pololetní písemka', 'Čtvrtletní práce', 'Aktivita v hodině', 'Referát',
    'Laboratorní práce', 'Čtenářský deník', 'Seminární práce', 'Diktát', 'Slohová práce',
];
...
interface SubjectDef { name: string; code: string; }
...
function getSubjectsForType(type: string): SubjectDef[] {
    switch (type) {
        case 'elementary_1': return SUBJECTS_ELEMENTARY_1;
        case 'elementary_full': {
            const merged = [...SUBJECTS_ELEMENTARY_1, ...SUBJECTS_ELEMENTARY_2];
            const seen = new Set<string>();
            return merged.filter(s => { if (seen.has(s.code)) return false; seen.add(s.code); return true; });
        }
        case 'gymnasium_8':
        case 'gymnasium_4': return SUBJECTS_GYMNASIUM;
        default: return SUBJECTS_ELEMENTARY_1;
    }
}

interface GradeDef { levelNumber: number; levelName: string; classrooms: string[]; }

function getGradesForType(type: string): GradeDef[] {
    switch (type) {
        case 'elementary_1':
            return Array.from({ length: 5 }, (_, i) => ({ levelNumber: i + 1, levelName: `${i + 1}. ročník`, classrooms: [`${i + 1}.A`, `${i + 1}.B`] }));
        case 'elementary_full':
            return Array.from({ length: 9 }, (_, i) => ({ levelNumber: i + 1, levelName: `${i + 1}. ročník`, classrooms: [`${i + 1}.A`, `${i + 1}.B`] }));
        case 'gymnasium_8': {
            const names = ['Prima', 'Sekunda', 'Tercie', 'Kvarta', 'Kvinta', 'Sexta', 'Septima', 'Oktáva'];
            return names.map((name, i) => ({ levelNumber: i + 1, levelName: name, classrooms: [`${name} A`, `${name} B`] }));
        }
        case 'gymnasium_4':
            return Array.from({ length: 4 }, (_, i) => ({ levelNumber: i + 1, levelName: `${i + 1}. ročník`, classrooms: [`${i + 1}.A`, `${i + 1}.B`] }));
        default: return [];
    }
}
...
const DEMO_PASSWORD_HASH = '$2b$10$8K1p/q5zQxl0SRDV4Gqe6eruJ3Mn1.Tl5Yng3ORq0q6Z8hMO0dPHG';

// ─── Config types ────────────────────────────────────────────────

export interface GenerateConfig {
    schoolName: string;
    schoolType: 'elementary_1' | 'elementary_full' | 'gymnasium_8' | 'gymnasium_4';
    teacherCount: number;
...
export class TestDataService {
    ...
    async generateAll(config: GenerateConfig): Promise<GenerateResult> {
        this.logger.log(`Generating test data for school '${config.schoolName}' (${config.schoolType})`);
        ...
        // 1. Create school
        const school = await this.prisma.school.create({ data: { name: config.schoolName } });
        const schoolId = school.id;

        // 2. Create structure
        const { academicYear, classroomIds, gradeLevelMap } = await this.createStructure(schoolId, config.schoolType, stats);

        // 3. Create users
        const teacherUserIds = await this.createTeachers(schoolId, config, academicYear?.id, stats);
        const { studentUserIds, studentProfileIds } = await this.createStudents(schoolId, config, classroomIds, academicYear, gradeLevelMap, stats);
        const parentUserIds = await this.createParents(schoolId, config, studentUserIds, stats);

        // 4. Create subjects
        let subjectInstanceIds: string[] = [];
        let teacherProfileIds: string[] = [];
        if (config.generateSubjects) {
            const result = await this.createSubjects(schoolId, config.schoolType, academicYear, gradeLevelMap, stats);
        ...
        const teacherProfiles = await this.prisma.teacherProfile.findMany({
            where: { userId: { in: teacherUserIds } }, select: { id: true },
        });
        teacherProfileIds = teacherProfiles.map(t => t.id);

        // 5. Schedule
        if (config.generateSchedule && subjectInstanceIds.length > 0 && teacherProfileIds.length > 0) {
            await this.createSchedule(schoolId, academicYear!, classroomIds, subjectInstanceIds, teacherProfileIds, stats);
        }

        // 6. Grades
        if (config.generateGrades && subjectInstanceIds.length > 0 && studentProfileIds.length > 0 && teacherProfileIds.length > 0) {
            await this.createGrades(schoolId, academicYear, subjectInstanceIds, studentProfileIds, teacherProfileIds, stats);
        }
    ...
    private async createStructure(schoolId: string, schoolType: string, stats: GenerateResult['stats']) {
        const grades = getGradesForType(schoolType);
        const yearName = '2025/2026';

        const academicYear = await this.prisma.academicYear.create({
            data: { name: yearName, startDate: new Date('2025-09-01'), endDate: new Date('2026-06-30'), isCurrent: true, schoolId },
        });
        ...
        await this.prisma.semester.createMany({
            data: [
                { number: 1, name: '1. pololetí', startDate: new Date('2025-09-01'), endDate: new Date('2026-01-31'), academicYearId: academicYear.id },
                { number: 2, name: '2. pololetí', startDate: new Date('2026-02-01'), endDate: new Date('2026-06-30'), academicYearId: academicYear.id },
            ],
        });
        ...
        // Create rooms
        const rooms = ['A101', 'A102', 'A103', 'B201', 'B202', 'B203', 'C301', 'PC1', 'Tělocvična', 'Aula'];
        for (const name of rooms) {
            await this.prisma.room.create({
                data: {
                    name, schoolId, capacity: name === 'Tělocvična' ? 60 : name === 'Aula' ? 100 : 30,
    ...
    private async createTeachers(schoolId: string, config: GenerateConfig, academicYearId: string | undefined, stats: GenerateResult['stats']): Promise<string[]> {
        ...
        for (let i = 0; i < totalTeachers; i++) {
            ...
            while (usedEmails.has(email)) { email = `${removeDiacritics(lastName)}${i}${randInt(1, 99)}@demo.edustack.cz`; }
            usedEmails.add(email);

            const isActive = i < config.teacherActiveCount;
            const isInvited = !isActive && i < config.teacherActiveCount + config.teacherInvitedCount;
            ...
            await this.prisma.schoolMembership.create({
                data: { userId: user.id, schoolId, role: 'TEACHER', status, workloadPercentage: Math.random() > 0.3 ? 1.0 : 0.5 + Math.random() * 0.5 },
            });

            await this.prisma.teacherProfile.create({
                data: { userId: user.id, degree: pick(TEACHER_DEGREES), approbation: pick(TEACHER_APPROBATIONS) },
            });

            if (academicYearId) {
                await this.prisma.teacherWorkload.create({
                    data: { teacherId: user.id, academicYearId, workloadPercentage: Math.random() > 0.3 ? 1.0 : 0.5 + Math.random() * 0.5 },
                });
            }
            stats.teachers++;
        }
    ...
    private async createStudents(
        schoolId: string, config: GenerateConfig,
        classroomIds: string[], academicYear: any, gradeLevelMap: Record<number, string>,
        ...
        for (let i = 0; i < totalStudents; i++) {
            ...
            while (usedEmails.has(email)) { email = `student.${removeDiacritics(lastName)}${i}${randInt(1, 99)}@demo.edustack.cz`; }
            usedEmails.add(email);

            const isActive = i < config.studentActiveCount;
            const isInvited = !isActive && i < config.studentActiveCount + config.studentInvitedCount;
            const status = isActive ? 'ACTIVE' : isInvited ? 'PENDING' : 'ACTIVE';
            ...
            if (academicYear && classroom) {
                ...
                if (gradeLevelId) {
                    await this.prisma.studentEnrollment.create({
                        data: { studentId: user.id, academicYearId: academicYear.id, gradeLevelId, classroomId },
                    });
                }
            }
    ...
    private async createParents(schoolId: string, config: GenerateConfig, studentUserIds: string[], stats: GenerateResult['stats']): Promise<string[]> {
        const parentUserIds: string[] = [];
        const parentCount = config.parentCount === 0 ? studentUserIds.length : config.parentCount;
        const usedEmails = new Set<string>();
        ...
        for (let i = 0; i < parentCount && i < studentUserIds.length; i++) {
            ...
            while (usedEmails.has(email)) { email = `rodic.${removeDiacritics(lastName)}${i}${randInt(1, 99)}@demo.edustack.cz`; }
            usedEmails.add(email);
    ...
    private async createSubjects(
        schoolId: string, schoolType: string, academicYear: any,
        gradeLevelMap: Record<number, string>, stats: GenerateResult['stats'],
        ...
        for (const subj of subjects) {
            ...
            if (academicYear) {
                for (const [levelNumStr, gradeLevelId] of Object.entries(gradeLevelMap)) {
                    const levelNum = parseInt(levelNumStr);
                    const hoursPerWeek = this.getHoursPerWeek(subj.code, levelNum, schoolType);
                    if (hoursPerWeek <= 0) continue;

                    const instance = await this.prisma.subjectInstance.create({
                        data: { templateId: template.id, academicYearId: academicYear.id, gradeLevelId, schoolId, hoursPerWeek },
                    });
    ...
    private getHoursPerWeek(code: string, level: number, _type: string): number {
        const base: Record<string, number> = {
            'CJL': 5, 'M': 4, 'AJ': 3, 'NJ': 2, 'FJ': 2,
            'F': 2, 'CH': 2, 'PŘ': 2, 'BI': 2, 'Z': 2, 'D': 2,
            'OV': 1, 'ZSV': 2, 'HV': 1, 'VV': 2, 'TV': 2,
            'PČ': 1, 'INF': 1, 'IVT': 2, 'PRV': 2, 'VL': 2,
        };
        return base[code] || 2;
    }
    ...
    private async createSchedule(
        ...
        subjectInstanceIds: string[], teacherProfileIds: string[],
        stats: GenerateResult['stats'],
    ) {
        const slots = await this.prisma.lessonTimeSlot.findMany({ where: { schoolId }, orderBy: { lessonNumber: 'asc' } });
                    ...
                    try {
                        await this.prisma.scheduleEvent.create({
                            data: {
                                ...
                                schoolId, subjectInstanceId: instance.id,
                                classroomId: classroom.id, teacherId: assignedTeacher,
    ...
    private async createGrades(
        ...
        const students = await this.prisma.studentProfile.findMany({
            ...
            select: { id: true, classroomId: true, classroom: { select: { grade: true } } },
        });
        ...
        for (const student of students) {
            ...
            for (const instance of relevantInstances) {
                ...
                for (let g = 0; g < gradeCount; g++) {
                    ...
                    const category = ['EXAM', 'HOMEWORK', 'CLASSWORK', 'PROJECT', 'OTHER'][randInt(0, 4)];
    ...
    private async createCommunication(
        ...
        for (let c = 0; c < Math.min(5, Math.floor(allUserIds.length / 2)); c++) {
            const senderId = teacherUserIds.length > 0 ? pick(teacherUserIds) : allUserIds[0];
            let recipientId = parentUserIds.length > 0 ? pick(parentUserIds) : pick(studentUserIds.length > 0 ? studentUserIds : allUserIds);
            ...
            for (let m = 0; m < msgCount; m++) {
                ...
                await this.prisma.message.create({
                    data: { conversationId: conversation.id, senderId: msgSenderId, content: pick(MESSAGE_CONTENTS), createdAt: msgDate },
                });
                stats.messages++;
            }
        }
        ...
        if (teacherUserIds.length > 0) {
            ...
            const broadcastParticipants = [broadcastSender, ...studentUserIds.slice(0, 10), ...parentUserIds.slice(0, 10)];
            ...
            if (uniqueParticipants.length >= 2) {
                const broadcast = await this.prisma.conversation.create({
                    data: {
                        ...
                        schoolId,
                        participants: { create: uniqueParticipants.map(uid => ({ userId: uid })) },
                    },
                });
                stats.conversations++;
                ...
                await this.prisma.message.create({
                    data: {
                        conversationId: broadcast.id, senderId: broadcastSender,
                        content: 'Vážení rodiče, oznamujeme, že třídní schůzky se budou konat v pondělí 10. března od 17:00 v prostorách školy. Těšíme se na setkání.',
                    },
                });
                stats.messages++;
            }
        ...
        for (const userId of allUserIds.slice(0, 20)) {
            await this.prisma.notification.create({
                data: {
                    ...
                    title: pick(['Nová zpráva', 'Nová známka', 'Systémové oznámení', 'Docházka']),
                    body: pick(['Máte novou zprávu od učitele.', 'Byla vám zadána nová známka.', 'Systém byl aktualizován.', 'Absence byla zaznamenána.']),
    ...
    async wipeSchoolData(schoolId: string): Promise<{ deletedSchool: string }> {
        ...
        this.logger.warn(`Wiping ALL data for school '${school.name}' (${schoolId})`);
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            for (const uid of userIds) {
                ...
                const user = await tx.user.findUnique({ where: { id: uid }, select: { isSystemAdmin: true } });
            ...
            await tx.parentStudent.deleteMany({ where: { OR: [{ parentId: { in: userIdsOnlyInThisSchool } }, { studentId: { in: userIdsOnlyInThisSchool } }] } });
            await tx.teacherProfile.deleteMany({ where: { userId: { in: userIdsOnlyInThisSchool } } });
            await tx.studentProfile.deleteMany({ where: { userId: { in: userIdsOnlyInThisSchool } } });
            await tx.identity.deleteMany({ where: { userId: { in: userIdsOnlyInThisSchool } } });
            ...
            await tx.notification.deleteMany({ where: { userId: { in: userIdsOnlyInThisSchool } } });
            await tx.user.deleteMany({ where: { id: { in: userIdsOnlyInThisSchool } } });
    ...
    async wipeAllData(): Promise<{ deletedSchools: number; deletedUsers: number }> {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            const deletedUsers = await tx.user.deleteMany({ where: { isSystemAdmin: false } });
