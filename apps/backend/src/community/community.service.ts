...
export class CommunityService {
  constructor(private prisma: PrismaService) {}
    ...
    async createBulletinPost(userId: string, schoolId: string, data: { title: string; content: string; pinned?: boolean }) {
    ...
    async updateBulletinPost(userId: string, schoolId: string, postId: string, data: { title?: string; content?: string; pinned?: boolean }) {
        const post = await this.prisma.bulletinPost.findFirst({ where: { id: postId, schoolId } });
    ...
    async createPoll(userId: string, schoolId: string, data: {
        question: string; options: string[]; multiSelect?: boolean; endsAt?: string;
    }) {
        if (!data.options || data.options.length < 2) throw new BadRequestException('Alespoň 2 možnosti');
    ...
    async deletePoll(schoolId: string, pollId: string) {
        const poll = await this.prisma.poll.findFirst({ where: { id: pollId, schoolId } });
    ...
    async createCalendarEvent(userId: string, schoolId: string, data: {
        title: string; description?: string; startDate: string; endDate?: string; location?: string;
    }) {
    ...
    async rsvpEvent(userId: string, eventId: string, status: 'YES' | 'NO' | 'MAYBE') {
    ...
    async deleteCalendarEvent(schoolId: string, eventId: string) {
        const event = await this.prisma.calendarEvent.findFirst({ where: { id: eventId, schoolId } });
        if (!event) throw new NotFoundException('Event not found');
