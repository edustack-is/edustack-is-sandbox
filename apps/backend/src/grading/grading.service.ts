...
export class GradingService {
  ...
  async getGradesForClassroom(
    ...
    const subjectFilter: any = { schoolId, classroomId };
    if (!opts?.isAdmin) {
      const teacherProfile = await this.getTeacherProfile(userId);
  ...
  async upsertReportCard(
    ...
    const reportCard = await this.prisma.reportCard.upsert({
      ...
      update: {
        ...
        ...(data.verbalEvaluation !== undefined && {
...
Vrať pouze upravený text hodnocení, bez dalšího komentáře.`;
  ...
  async upsertBehaviorGrade(
    ...
    if (data.grade < 1 || data.grade > 3)
      throw new BadRequestException('Behavior grade must be 1-3.');
  ...
  async upsertCompetencyGrade(
    ...
    if (data.level < 1 || data.level > 4)
      throw new BadRequestException('Competency level must be 1-4.');
  ...
  async getMeasures(
    schoolId: string,
    filters?: { classroomId?: string; studentId?: string; semesterId?: string },
  ) {
  ...
  async deleteMeasure(schoolId: string, id: string) {
    const measure = await this.prisma.educationalMeasure.findFirst({
      where: { id, schoolId },
    });
  ...
  async getReportCardHtml(
    ...
    const data = await this.getReportCardsForClass(
      schoolId,
      classroomId,
      semesterId,
    );
    ...
    for (const student of data.students) {
      ...
      for (const subj of data.subjects) {
        const subjData = student.subjects.find(
          (s: any) => s.subjectInstanceId === subj.id,
        );
  ...
  async getCommissionExams(
    schoolId: string,
    filters?: { classroomId?: string; semesterId?: string },
  ) {
    const where: any = { schoolId };
    if (filters?.semesterId) where.semesterId = filters.semesterId;
    if (filters?.classroomId) {
  ...
  async updateCommissionExam(
    schoolId: string,
    id: string,
    data: { newGrade?: string; note?: string; date?: string },
  ) {
    const exam = await this.prisma.commissionExam.findFirst({
      where: { id, schoolId },
    });
  ...
  async upsertDeadline(
    schoolId: string,
    data: { semesterId: string; deadline: string; isLocked?: boolean },
  ) {
