...
export class TeacherController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Rozvrh učitele napříč školami – matice.',
  ...
  @ApiResponse({
    status: 200,
    description: 'Třídy a studenti učitele – pole.',
    type: TeacherClassResponseDto,
    isArray: true,
  })
  @ApiResponse({
    status: 401,
  ...
  async createGrade(
    ...
    this.ensureTenantContext(req);
    return this.teacherService.createGrade(
      req.user.userId,
      req.user.schoolId,
      body,
    );
  ...
  async createAttendance(
    ...
    this.ensureTenantContext(req);
    return this.teacherService.createAttendance(
      req.user.userId,
      req.user.schoolId,
      body,
    );
