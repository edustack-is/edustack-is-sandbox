...
export class AttendanceService {
    ...
    async recordAttendance(userId: string, schoolId: string, data: {
        date: string;
        lessonNumber: number;
        classroomId: string;
        ...
        for (const rec of data.records) {
            ...
            if (rec.status === 'ABSENT') {
                await this.notifyParentsAboutAbsence(rec.studentId, data.date, data.lessonNumber);
            }
        }
        return results;
    }
    ...
    async getClassroomAttendance(schoolId: string, classroomId: string, date: string) {
        const classroom = await this.prisma.classroom.findUnique({
    ...
    async createExcuse(parentId: string, schoolId: string, data: {
        studentId: string; reason: string; dateFrom: string; dateTo: string;
    }) {
        ...
        if (!relation) throw new BadRequestException('You are not a parent of this student');
    ...
    async getExcuses(schoolId: string, filters?: { classroomId?: string; status?: string }) {
    ...
    async reviewExcuse(userId: string, schoolId: string, excuseId: string, status: 'APPROVED' | 'REJECTED') {
        const excuse = await this.prisma.absenceExcuse.findFirst({ where: { id: excuseId, schoolId } });
    ...
    async getClassStatistics(schoolId: string, classroomId: string, dateFrom?: string, dateTo?: string) {
        const classroom = await this.prisma.classroom.findUnique({
            where: { id: classroomId },
            include: {
                students: {
    ...
    async exportAttendanceCsv(schoolId: string, classroomId: string, dateFrom?: string, dateTo?: string) {
        const data = await this.getClassStatistics(schoolId, classroomId, dateFrom, dateTo);
        const BOM = '\uFEFF';
        let csv = BOM + 'Student;Celkem;Přítomen;Nepřítomen;Pozdě;Omluven;% přítomnost\n';
    ...
    private async notifyParentsAboutAbsence(studentId: string, date: string, lessonNumber: number) {
