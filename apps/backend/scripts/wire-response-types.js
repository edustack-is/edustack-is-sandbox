...
for (const [relPath, methods] of Object.entries(MAP)) {
    ...
    if (fromResp.size > 0) {
        ...
        if (!content.includes("from '" + RESPONSE_FILE + "'")) {
            ...
            content = content.slice(0, nl2 + 1) +
                "import { " + items + " } from '" + RESPONSE_FILE + "';\n" +
                content.slice(nl2 + 1);
            mod = true;
        }
    }
    ...
    for (const [methodName, resp] of Object.entries(methods)) {
        ...
        for (const code of ['200', '201']) {
            ...
            const newSnippet = snippet.replace(' })', ', type: ' + resp.t + arrPart + ' })');

            const absStart = areaStart + respStart;
            content = content.slice(0, absStart) + newSnippet + content.slice(absStart + snippet.length);
            totalWired++;
            mod = true;
            break;
        }
