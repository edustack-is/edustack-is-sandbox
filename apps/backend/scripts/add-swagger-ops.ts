...
const PUBLIC_CONTROLLERS = new Set(['system-admin/monitoring.controller.ts']);
...
for (const [relPath, methods] of Object.entries(OPS)) {
    ...
    if (!content.includes(imp)) {
      if (content.includes("from '@nestjs/swagger'")) {
        content = content.replace(
          ...
          (match, imports) =>
            `import {${imports}, ${imp} } from '@nestjs/swagger'`,
        );
        fileModified = true;
      }
    }
  ...
  for (const [methodName, summary] of Object.entries(methods)) {
    ...
    if (match) {
      ...
      const isPublic =
        PARTIAL_PUBLIC_METHODS.has(methodName) ||
        PUBLIC_CONTROLLERS.has(relPath);
      ...
      if (
        /create|upsert|update|save|record|invite|import|upload|setup|vote|review|send|toggle|bulk|batch|promote|demote|share|generate|confirm|sign|rsvp/i.test(
          methodName,
        )
      ) {
