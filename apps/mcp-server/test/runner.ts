...
async function runTests() {
    ...
    const httpServer = app.listen(PORT, async () => {
        try {
            ...
            assert.ok((createResult as any).content[0].text.includes("byla úspěšně vytvořena"), "Should contain success message");
            ...
            const valResult = await mcpClient.callTool({
                name: "create_school",
                arguments: {}
            }).catch(e => e);
            ...
            assert.ok((seedResult as any).content[0].text.includes("byla úspěšně naplněna strukturou"), "Should contain success message");
