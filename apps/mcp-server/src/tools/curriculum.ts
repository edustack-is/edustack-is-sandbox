...
server.tool(
    ...
    async ({ schoolId, name, startDate, endDate, isCurrent }) => {
        try {
            ...
            if (!school) {
                return { isError: true, content: [{ type: "text", text: `Škola s ID '${schoolId}' nebyla nalezena.` }] };
            }
            ...
            if (existing) {
                return { isError: true, content: [{ type: "text", text: `Školní rok '${name}' pro tuto školu již existuje (ID: ${existing.id}).` }] };
            }
...
server.tool(
    ...
    async ({ schoolId }) => {
        try {
            ...
            if (levels.length === 0) {
                return { content: [{ type: "text", text: "Škola nemá žádné ročníky. Použij seed_school_structure pro vytvoření." }] };
            }
...
server.tool(
    ...
    async ({ schoolId }) => {
        try {
            ...
            return { content: [{ type: "text", text: `Šablony předmětů (${templates.length}):\n${lines.join("\n")}` }] };
        } catch (error: any) {
            return { isError: true, content: [{ type: "text", text: `Chyba: ${error.message}` }] };
        }
    }
...
server.tool(
    ...
    async ({ schoolId, name, code, svpDescription }) => {
        try {
            ...
            if (existing) {
                return { isError: true, content: [{ type: "text", text: `Předmět s kódem '${code}' v této škole již existuje (ID: ${existing.id}).` }] };
            }
...
server.tool(
    ...
    async ({ schoolId, templateId, academicYearId, gradeLevelId, hoursPerWeek }) => {
        try {
            ...
            if (!template) return { isError: true, content: [{ type: "text", text: `Šablona předmětu '${templateId}' nebyla nalezena.` }] };
            if (!year) return { isError: true, content: [{ type: "text", text: `Školní rok '${academicYearId}' nebyl nalezen.` }] };
            if (!grade) return { isError: true, content: [{ type: "text", text: `Ročník '${gradeLevelId}' nebyl nalezen.` }] };
            ...
            if (existing) {
                return { isError: true, content: [{ type: "text", text: `Instance '${template.name}' pro ${grade.name} v ${year.name} již existuje (ID: ${existing.id}).` }] };
            }
...
server.tool(
    ...
    async ({ schoolId, academicYearId, gradeLevelId }) => {
        try {
            ...
            return { content: [{ type: "text", text: `Instance předmětů (${instances.length}):\n${lines.join("\n")}` }] };
        } catch (error: any) {
            return { isError: true, content: [{ type: "text", text: `Chyba: ${error.message}` }] };
        }
    }
    ...
    async ({ schoolId, studentIds, academicYearId, gradeLevelId, classroomId }) => {
        try {
            ...
            if (classroomId) {
                ...
                if (!classroom) return { isError: true, content: [{ type: "text", text: `Třída '${classroomId}' nebyla nalezena.` }] };
            }
